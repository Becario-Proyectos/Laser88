...
# --- Stub para sincronizar_pendientes_a_mysql si no existe ---
    ...
    for linea in pendientes:
        try:
            ...
            if len(partes) < 5:
                continue
            fecha = partes[0].strip()
            # El nombre puede tener comas, así que buscar el índice del número
            num_index = -1
...
# --- Llamada a sincronizar_todo_pendiente() al final del archivo ---
if __name__ == "__main__":
    # Sincroniza pendientes al iniciar
    sincronizar_todo_pendiente()
...
# REVISAR DUPLICADOS (ahora lee datos descifrados) para registro de personal
def check_duplicate_personal(nombre_nuevo, numero_registro_nuevo, codigo_nuevo):
    # Obtener los registros descifrados para la verificación
    registros_existentes = leer_registros_descifrados(NOMBRE_ARCHIVO_REGISTROS)

    # Si leer_registros_descifrados devolvió un error (lista que comienza con "ERROR:"),
    # entonces no podemos verificar duplicados de forma fiable.
    if registros_existentes and registros_existentes[0].startswith("ERROR:"):
        ...
        # Es más seguro bloquear el registro para evitar duplicados no deseados si no se puede verificar.
        return True, "No se pudo verificar duplicados debido a un error en la lectura de la base de datos TXT."
    
    # Si la lista de registros está vacía, significa que el archivo no existía o estaba vacío.
    ...
    for line in registros_existentes:
        line = line.strip()
        # Ignorar líneas vacías o con errores de lectura/descifrado (si leer_registros_descifrados las incluyó)
        ...
        try:
            ...
            nombre_existente_parts = parts[1:num_index] # Nombre está entre fecha y numero
            nombre_existente = ",".join(nombre_existente_parts).strip()

            numero_registro_existente = parts[num_index].strip()
            # La contraseña estaría en num_index + 1, y el Code Laser en num_index + 2
...
def dar_de_baja_personal():
    ...
    try:
        ...
        if eliminado_txt:
            guardar_registro_cifrado(eliminado_txt, NOMBRE_ARCHIVO_BAJAS_PENDIENTES)
            #messagebox.showwarning("MySQL no disponible", "No se pudo eliminar en MySQL. La baja se guardó temporalmente y se sincronizará automáticamente cuando vuelva la conexión.")
            messagebox.showinfo("Éxito", f"Se dio de baja correctamente al número {numero_int}.")
        else:
            messagebox.showerror("Error", "No se encontró el registro a dar de baja.")
        return
...
def insert_data_into_mysql_partes(fecha, hora, numero_parte, numero_cat):
    ...
    try:
        ...
        #messagebox.showerror("Error de MySQL", f"No se pudo insertar en la 444base de datos MySQL (partes): {err}\nError original: {err.msg}")
...
def registrar_datos_personal():
    ...
    # Convertir numero_registro a INT para MySQL
    try:
...
def close_options_window():
    ...
    if options_window:
        ...
        options_window = None # Reiniciar para que pueda ser creada de nuevo
...
def show_login_screen_toplevel(parent_root):
    ...
    login_window.grab_set() # Hace que esta ventana sea modal
    ...
    # --- Área invisible para acceso rápido de desarrollador (esquina superior derecha) ---
    ...
    def attempt_login():
        ...
        if username == USUARIO_ADMIN and password == PASSWORD_ADMIN:
            login_successful = True # Marcar login como exitoso
            login_window.grab_release() # Liberar el grab_set antes de destruir
            login_window.destroy()
            show_options_screen() # Muestra la nueva ventana de opciones
        else:
...
def _overwrite_encrypted_file(lines, archivo_destino):
    """Sobrescribe el archivo TXT cifrado con las líneas provistas."""
    key = load_key()
    if key is None:
        messagebox.showerror("Error", "No se pudo generar/cargar la clave de cifrado para regenerar el TXT.")
        return False
...
def _get_mysql_conn_for_reads():
    """Conexión directa a la DB para lecturas garantizadas."""
...
def sync_txt_from_mysql_personal():
    ...
    # Formato de línea usado por tu app: fecha,nombre,numero,password,codigo
    lines = []
    for r in rows:
...
def sync_txt_from_mysql_partes():
    """Reconstruye DB_Partes.txt (cifrado) desde MySQL."""
    try:
        conn = _get_mysql_conn_for_reads()
        cur = conn.cursor()
    ...
    # Formato de línea que usas: fecha,hora,numero_parte,numero_cat
    lines = []
    for r in rows:
        fecha_str = r[0].strftime("%Y-%m-%d") if isinstance(r[0], date) else str(r[0])
...
def show_options_screen():
    ...
    # Cuando se selecciona una opción, se libera el grab_set de options_window
    # y la ventana principal toma el control.
...
def show_parts_registration_screen():
    ...
    def on_close_parts_window():
        # Solo la tachita termina la app
        exit_app()
    ...
    button_frame_parts.pack(pady=20)

    tk.Button(button_frame_parts, text="Registrar Núm Parte",
              command=registrar_datos_partes, bg=COLOR_SUCCESS, fg=COLOR_TEXT_LIGHT,
              font=FONT_BUTTONS, width=19, height=2, cursor="hand2", bd=2, relief="raised").pack(side="left", padx=10)

    tk.Button(button_frame_parts, text="Ver Base de Datos",
              command=show_parts_database_screen, bg=COLOR_ACCENT, fg=COLOR_TEXT_LIGHT,
              font=FONT_BUTTONS, width=18, height=2, cursor="hand2", bd=2, relief="raised").pack(side="left", padx=10)
    ...
    tk.Button(button_frame_parts, text="Dar de Baja",
              command=dar_de_baja_parte,
              bg=COLOR_ORANGE, fg=COLOR_TEXT_LIGHT, font=FONT_BUTTONS, width=18, height=2,
              cursor="hand2", bd=2, relief="raised").pack(side="left", padx=10)
    ...
    tk.Button(regresar_frame_parts, text="Regresar a Opciones",
              command=back_to_options,
              bg=COLOR_DANGER, fg=COLOR_TEXT_LIGHT, font=FONT_BUTTONS, width=18, height=2,
              cursor="hand2", bd=2, relief="raised").pack(side="left", padx=10)
    
# --- DAR DE BAJA EN LA DB MYSQL (PARTES) - VERSIÓN OFFLINE/ONLINE ROBUSTA ---
def dar_de_baja_parte():
    ...
    try:
        ...
        guardar_registro_cifrado(eliminado_txt, NOMBRE_ARCHIVO_BAJAS_PARTES_PENDIENTES)
        #messagebox.showwarning("MySQL no disponible", "No se pudo eliminar en MySQL. La baja se guardó temporalmente y se sincronizará automáticamente cuando vuelva la conexión.", parent=parts_window)
        messagebox.showinfo("Éxito", f"Se dio de baja correctamente el número CAT {numero_cat_a_borrar}.", parent=parts_window)
        return
...
def ensure_txt_for_write(path, cache_lines, sync_fn):
    ...
    # Si ya existe con datos, nada que hacer
    if os.path.exists(path) and os.path.getsize(path) > 0:
        return

    # 1) MySQL
    try:
        if sync_fn():  # True si pudo escribir el TXT completo cifrado
            return
    except Exception:
        pass
...
def show_parts_database_screen():
    ...
    # (Header eliminado: no mostrar franja azul ni texto superior)
    ...
    btn_clear = tk.Button(search_frame, text="Limpiar", command=limpiar_search, font=(FONT_BUTTONS[0], 10, "bold"), bg=COLOR_DANGER, fg=COLOR_TEXT_LIGHT, width=8, height=1)
    btn_clear.pack(side="left", padx=(0, 5))
    # Botón cerrar a la derecha de limpiar (bien pegado y visible)
    ...
    # El botón cerrar ya está en la barra de búsqueda
...
def setup_main_app_widgets(main_window):
    ...
    # --- Frame para el Header con Texto e Imagen ---
    ...
    button_frame_bottom.pack(pady=(0, 10))  # Margen inferior


    # IMPORTANTE: Al regresar a opciones o salir, liberar el grab_set if estuviera activo - ES EL DE PANTALLA PERSONAL
...
#ASEGURAR TXT AL INICIO (Y CUANDO FALTEN)
def ensure_txt_files_from_mysql():
    """Si faltan los TXT, los reconstruye cifrados desde MySQL."""
    # Personal
...
if __name__ == "__main__":
    ...
    # Sincronizar  lo pendiente (registros y bajas) antes de cualquier otra cosa
    sincronizar_todo_pendiente()
